  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const currentVisitorAvailable = await checkEndpointAvailability(
        ...
        const activeVisitorsAvailable = await checkEndpointAvailability(
          "/api/visitors/active"
        );
        ...
        if (isMounted) {
          ...
          setError(
            err.response?.data?.message || "Failed to fetch visitor data"
          );
          setVisitorData(null);
          setActiveVisitors([]);
        }
