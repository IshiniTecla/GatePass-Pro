...
import NewVisitor from "./hosts/NewVisitor"; 
import HostNetwork from "./hosts/HostNetwork";
import CreateMeeting from "./meeting/CreateMeetingPage";
import ScheduledSessions from "./meeting/ScheduledSessions";
...
const HostDashboard = () => {
  ...
  const getActivePageFromPath = (path) => {
    ...
    if (path.includes("/history") && !path.includes("/meeting-history")) return "visitationHistory";
    if (path.includes("/meeting") && !path.includes("/meeting-history") && !path.includes("/scheduled-sessions")) return "createMeeting";
    if (path.includes("/scheduled-sessions")) return "scheduledSessions";
  ...
  const sidebarItems = [
    ...
    { id: "scheduledSessions", label: "Scheduled Sessions", icon: <FaCalendarWeek />, path: "/host-dashboard/scheduled-sessions" },
          ...
          <Routes>
            ...
            <Route path="meeting-room/:meetingId" element={
              <React.Suspense fallback={<div>Loading meeting room...</div>}>
                {React.createElement(React.lazy(() => import('./meeting/MeetingRoom')))}
              </React.Suspense>
            } />
            <Route path="meetings/:meetingId" element={
              <React.Suspense fallback={<div>Loading meeting details...</div>}>
                {React.createElement(React.lazy(() => import('./meeting/MeetingDetails')))}
              </React.Suspense>
            } />
          </Routes>
        </div>
