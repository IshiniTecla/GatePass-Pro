...
import { sendMeetingInvitation } from "../utils/emailMeetingService.js";
...
const createMeeting = async (req, res) => {
  try {
    ...
    if (!host) {
      return res.status(403).json({ message: "Only hosts can create meetings" });
    }

    const { title, description, startTime, endTime, password, recordingEnabled, participants } = req.body;
...
const updateMeeting = async (req, res) => {
  try {
    ...
    if (status && ["scheduled", "cancelled"].includes(status)) meeting.status = status;
