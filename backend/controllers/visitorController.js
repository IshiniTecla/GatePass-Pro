...
export const createVisitor = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Server error while creating visitor" });
  }
};
...
export const getAllVisitors = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Server error while fetching visitors" });
  }
};
  ...
  try {
    ...
    if (visitorId === "current") {
      ...
      if (!visitor) {
        return res
          .status(404)
          .json({ success: false, message: "No current visitor found" });
      }
    ...
    if (visitorId === "recent") {
      ...
      if (!visitor) {
        return res
          .status(404)
          .json({ success: false, message: "No recent visitor found" });
      }
    ...
    visitor = await Visitor.findById(visitorId).populate(
      "host",
      "name department"
    );
...
export const updateVisitor = async (req, res) => {
  try {
    ...
    const updatedVisitor = await Visitor.findByIdAndUpdate(visitorId, updates, {
      new: true,
    }).populate("host", "name department");
    ...
    return res
      .status(500)
      .json({ error: "Server error while updating visitor" });
  }
};
...
export const deleteVisitor = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Server error while deleting visitor" });
  }
};
...
export const getCurrentVisitor = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Server error while retrieving visitor",
    });
  }
};
